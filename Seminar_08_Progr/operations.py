...
def db_parse(): #парсинг базы. читает файл и возвращает содержимое в виде list1 = [[di1,imya1,fam2, ...], [di2,imya2,fam, ...], [di3,imya3,fam, ...], ...] """
...
def db_input(user_data): # дописывает в конец базы строку
...
# [di2,imya2,fam], [di3,imya3,fam]] проходит по всем элементам списка списков, ища совпадения, 
# что нашел отправил в список списков НО ДРУГОЙ

def db_search(string_from_user): # поиск по базе
...
def db_item_del(id_from_user): # удаление записи
...
def db_edit(id_from_user,list_from_user): # редактирование записи
...
def list_write(list_for_rewrite): # формирует и перезаписывает базу из списка вида [[1,11,111],[2,22,222]]
...
def id_num(): # отдает количество айдишников в базе (для проверки ввода числа айдишников)
...
def convert_dics_to_list(data: list[dict[str, str]]) -> list[list[str]]: # конвертирует данные при импорте
