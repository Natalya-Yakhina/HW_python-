...
def push_the_batton():
    ...
    while True:
        ...
        command = inp_string("Выберите комманду (1 - создание записи, 2 - поиск, 3 - удаление, 4 - редактирование,\n 5 - всех записей, 6 - импорт, 7 - экспорт, 8 - выход) ")
        
        if command == "1": # внесение записи
            user_data = inp_list()
        ...
        elif command == "3": # удаление записи
        ...
        elif command == "5": # просмотр базы
            resive_data = operations.db_parse()
            ui.view(resive_data)
            logger.log_error("Просмотр всей базы")

        elif command == "6": # импорт
            try:
